...
export default function PrincipalAluno() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const [pontos] = useState(Number(params.pontos || 0));
  ...
  useFocusEffect(
    useCallback(() => {
      async function carregarDados() {
        try {
          const dadosSalvos = await AsyncStorage.getItem("@movimentacoes_aluno");
          if (dadosSalvos) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: Cores.fundo }]}>
      <View style={[styles.topoAluno, { backgroundColor: Cores.botao }]}>
        <View>
          <Text style={[styles.nomeAluno, { color: Cores.textoBotao }]}>
            Olá, Aluno(a) 
          </Text>
          <Text style={[styles.subtituloAluno, { color: Cores.textoBotao }]}>
            Painel For Schools
          </Text>
        </View>
      ...
      <View style={[styles.menuInferior, { backgroundColor: Cores.botao }]}>
        <TouchableOpacity onPress={() => router.push("/atividadesaluno")}>
          <Ionicons name="book" size={26} color={Cores.textoBotao} style={{ alignSelf: "center" }} />
          <Text style={[styles.menuTexto, { color: Cores.textoBotao }]}>Atividades</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  topoAluno: { padding: 20, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  nomeAluno: { fontSize: 20, fontWeight: "bold" },
  subtituloAluno: { fontSize: 12, opacity: 0.9 },
  caixaPontos: { backgroundColor: "rgba(0,0,0,0.2)", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, flexDirection: "row", alignItems: "center", gap: 6 },
  textoPontos: { color: "#fff", fontWeight: "bold" },
